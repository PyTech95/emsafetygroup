...
export default function ContactSection({ onSubmitted, prefill }: ContactProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSubmitted(t('home.contact.success'));
      setForm({ name: '', company: '', email: '', phone: '', service: '', message: '' });
      setProfileAdded(false);
      lastPrefill.current = '';
    } catch {
      onSubmitted('Errore durante l\u2019invio. Riprova o scrivici a ' + t('company.email'));
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <section id="contatti" className="bg-slate-50 py-20 sm:py-24 lg:py-28 border-t border-slate-200" data-testid="contact-section">
      <div className="max-w-[1320px] 2xl:max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-12 gap-6 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-5 space-y-5">
            <div className="bg-[#f0f9ff] text-neutral-900 rounded-3xl p-6 sm:p-7 shadow-xl border border-sky-100">
              <div className="flex items-center gap-2 font-display text-[19px] font-bold">
                <Building2 className="w-5 h-5 text-[#1e6fd9]" /> {t('company.name')}
              </div>

              <div className="mt-6 space-y-5">
                <div className="flex items-start gap-3.5">
                  <MapPin className="w-5 h-5 text-[#1e6fd9] mt-0.5 shrink-0" />
                ...
                <a href={COMPANY.phoneHref} data-testid="contact-phone" className="flex items-center gap-3.5 group">
                  <Phone className="w-5 h-5 text-[#1e6fd9] shrink-0" />
                  <span className="font-sans text-[14px] text-slate-600 group-hover:text-sky-700 transition-colors">{t('company.phone')}</span>
                </a>
                <a href={COMPANY.mobileHref} data-testid="contact-mobile" className="flex items-center gap-3.5 group">
                  <Smartphone className="w-5 h-5 text-[#1e6fd9] shrink-0" />
                  <span className="font-sans text-[14px] text-slate-600 group-hover:text-sky-700 transition-colors">{t('company.mobile')}</span>
                </a>
                <a href={`mailto:${t('company.email')}`} data-testid="contact-email" className="flex items-center gap-3.5 group">
                  <Mail className="w-5 h-5 text-[#1e6fd9] shrink-0" />
                  <span className="font-sans text-[14px] text-slate-600 group-hover:text-sky-700 transition-colors break-all">{t('company.email')}</span>
                </a>

                <div className="h-px bg-sky-200" />

                <div className="flex items-start gap-3.5">
                  <Clock className="w-5 h-5 text-[#1e6fd9] mt-0.5 shrink-0" />
                  <span className="font-sans text-[13.5px] text-slate-600 leading-relaxed">{t('company.hours')}</span>
                </div>
              </div>
              ...
              <div className="mt-6 pt-5 border-t border-sky-200 flex items-center gap-2 font-sans text-[12px] text-slate-400">
                ...
                P.IVA {t('company.vat')} &middot; REA {t('company.rea')}
              </div>
            </div>
          </div>
          ...
          <motion.form
            ...
            className="lg:col-span-7 bg-white rounded-3xl p-5 sm:p-8 shadow-xl border border-slate-100"
          >
            ...
            >
              {sending ? (<><Loader2 className="w-4 h-4 animate-spin" /> Invio in corso…</>) : (<>{t('home.contact.submit')}<Send className="w-4 h-4" /></>)}
            </button>
