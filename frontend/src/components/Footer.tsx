...
export default function Footer() {
  ...
  return (
    <footer className="w-full bg-slate-50 text-slate-600 border-t-4 border-neutral-900" data-testid="footer">
      <div className="max-w-[1320px] mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 pb-12 border-b border-slate-200">
          <div className="lg:col-span-5 flex flex-col gap-4">
            ...
            <p className="font-sans text-[13.5px] text-slate-500 max-w-md leading-relaxed">
              Società di consulenza e formazione per la salute e sicurezza nei luoghi di lavoro e la conformità
              normativa d&apos;impresa ai sensi del D.Lgs 81/08 e degli Accordi Stato-Regioni.
            </p>
          </div>
          ...
          <div className="lg:col-span-4 flex flex-col gap-2.5 select-text">
            ...
            <p className="font-sans text-[13.5px] text-slate-600 leading-relaxed"><strong className="text-neutral-900">Sede Legale:</strong> {COMPANY.legalOffice}</p>
            <p className="font-sans text-[13.5px] text-slate-600 leading-relaxed"><strong className="text-neutral-900">Sede Operativa:</strong> {COMPANY.operativeOffice}</p>
            <p className="font-sans text-[13.5px] text-slate-300 pt-1">
              <strong className="text-neutral-900">Tel:</strong>{' '}
              <a href={COMPANY.phoneHref} className="text-sky-700 hover:text-neutral-900 transition-colors font-semibold">{COMPANY.phone}</a>{' '}·{' '}
              <a href={COMPANY.mobileHref} className="text-sky-700 hover:text-neutral-900 transition-colors font-semibold">{COMPANY.mobile}</a>
            </p>
            <p className="font-sans text-[13.5px] text-slate-300"><strong className="text-neutral-900">Email:</strong>{' '}
              <a href={`mailto:${COMPANY.email}`} className="text-slate-600 hover:text-sky-700 transition-colors">{COMPANY.email}</a></p>
            <p className="font-sans text-[13.5px] text-slate-300"><strong className="text-neutral-900">PEC:</strong>{' '}
              <a href={`mailto:${COMPANY.pec}`} className="text-slate-600 hover:text-sky-700 transition-colors break-all">{COMPANY.pec}</a></p>
          </div>
        ...
        <div className="pt-6 flex flex-col md:flex-row items-center justify-between gap-4 font-sans text-[12px] text-slate-500 select-text">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-center md:text-left">
            ...
            <span>P.IVA {COMPANY.vat}</span><span>·</span><span>N° REA {COMPANY.rea}</span>
          </div>
